...
def get_model_params(datasets_path, dataset_name, model_type=None):
  ...
  obj_ids = {
    ...
    'fractal_dataset_v3_refined_pbr': [1],
    'fractal_tiny': [1]
  }[dataset_name]
  ...
  if dataset_name == 'tless' and model_type is None:
    model_type = 'cad'
  if dataset_name=='fractal_dataset_v3_refined_pbr':
    model_type = 'cad'
  if dataset_name=='fractal_tiny':
    model_type = 'cad'
  # Both versions of the HB dataset share the same directory.
  if dataset_name == 'hbs':
    dataset_name = 'hb'
...
def get_split_params(datasets_path, dataset_name, split, split_type=None):
  ...
  elif dataset_name == 'fractal_dataset_v3_refined_pbr':
    if split == "train_pbr":
      p["scene_ids"] = {
      "train_pbr": list(range(21)),
      "val": [],
      "test": [],
}[split]
    
    p["im_size"] = (848, 480)
  elif dataset_name == 'fractal_tiny':
    if split == "train_pbr":
      p["scene_ids"] = {
      "train_pbr": [1],
      "val": [],
